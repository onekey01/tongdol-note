...
"""
── 지은 문서는 **프로젝트 뿌리**에 놓습니다 (2026-09-11) ──────

  `build.ps1` 은 배포본에 담을 문서를 **프로젝트 뿌리**에서 집어 옵니다 —

      $문서들 = @("설치 안내.html", "사용설명서.html")

  그런데 이 파일은 여태 **자기 옆(문서짓기\)에만** 써 놓았습니다.
  그래서 화면 그림을 넣어 새로 지은 설명서가 `문서짓기\` 에만 쌓이고,
  배포본에는 **9월 8일자 옛 문서**가 그대로 실려 나갔습니다.
  1.21.36 이 그렇게 나갔습니다 — 「그림이 들어갔습니다」라고 적힌
  바뀐것 줄과 달리, 꾸러미 안의 문서에는 그림이 없었습니다.

  이제 **뿌리에 씁니다.** 집어 가는 자리와 놓는 자리를 같게 둡니다.
  (설명서시험.ts 가 뿌리 쪽 문서에 그림이 들어 있는지 셉니다.)
"""
나갈뿌리 = 뿌리.parent
...
"""
    (나갈뿌리 / 파일).write_text(글, encoding="utf-8")
    print(f"  {파일}  —  {len(글):,} 글자")
